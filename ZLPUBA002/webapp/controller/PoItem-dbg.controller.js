sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	return BaseController.extend("com.cu.s4hana.zlpuba002.controller.PoItem", {
		...
		onInit: function () {
			...
			this.oRouter.getTarget("PoItem").attachDisplay(jQuery.proxy(this.handleRouteMatched, this));
		...
		handleRouteMatched: function (oEvent) {
			...
			// Check if PO Item already initiated from PO Header Page
			// Otherwise -> Initial PO Item model and retrieve data from backend
			if (IsExisting) {
				this.syncData(po, poItem);
				this.getMasterData();
			} else {
				this.getData(po, poItem);
			}

			// Check mode already initiated from PO Header Page
			var controlModel = this.getOwnerComponent().getModel("controlModel");
			var vMode = "Display";
			if (controlModel && controlModel.getData().mode === "EDIT") {
				vMode = "Edit";
			}
			this._showFormFragment(vMode);
			...
			setTimeout(function () {
				for (var i = 0; i < poItemData.length; i++) {
					if (parseInt(poItemData[i].EBELP, 0) === parseInt(poItem, 0)) {
						...
						// if(poItemData[i].NOLIMIT === ''){
						// 	poItemData[i].NOLIMIT = false;
						// }else{
						// 	poItemData[i].NOLIMIT = true;
						// }
		...
		syncMessagePopover: function (poItem) {
			...
			for (var i = 0; i < messageData.length; i++) {
				...
				if (parseInt(items[1], 0) === parseInt(poItem, 0)) {
					var target = "";
					var controlId = "";
					var processor = "";
			...
			this.oMP = new sap.m.MessagePopover({
				activeTitlePress: function (oEvent) {
					var oItem = oEvent.getParameter("item"),
						oPage = that.oView.byId("idPoItemPage"),
			...
			var addAssetDialog = new sap.m.Dialog({
				...
				content: [
					new sap.m.HBox({
						...
						items: [
							new sap.m.Label({
								text: that.getView().getModel("i18n").getProperty("Label.ConfirmAddQty"),
								labelFor: 'idAddAsset'
							}),
		...
		doHandleAddAsset: function (val) {
			...
			var poItem_model = this.C_MODEL.poItemDetail;
			var poItemData = poItem_model.getData();
			var model = this.C_MODEL.poAccountAssignment;
			var data = model.getData();
			var lv_menge = 0; //++CH03

			//Get max line
			// var currentItem = data.length; //DEL BY Ch03
			//<BOI CH03
			var lastItem = data.length - 1;
			var currentItem = data[lastItem].ZEKKN;
			//>EOI CH03
			...
			var poItemDetail = this.getView().getModel("poItemDetail");
			var poItemDetailData = poItemDetail.getData();
			for (var i = 0; i < poItemData.length; i++) {
				if (parseInt(poItemData[i].EBELP, 0) === parseInt(data[1].EBELP, 0)) {
					for (var j in poItemData[i].PoAccAssignment.results) {
						lv_menge = lv_menge + parseInt(poItemData[i].PoAccAssignment.results[j].MENGE);
					}
					if (lv_menge) {
						poItemData[i].MENGE = lv_menge;
						//recal amount
						if (poItemData[i].PEINH == 1) {
							poItemData[i].RLWRT = poItemData[i].MENGE * poItemData[i].NETPR;
						} else {
							poItemData[i].RLWRT = (poItemData[i].MENGE / poItemData[i].PEINH) * poItemData[i].NETPR;
						}
					}
					if (poItemDetailData.EBELP == poItemData[i].EBELP) {
						poItemDetailData.MENGE = poItemData[i].MENGE;
						poItemDetailData.RLWRT = poItemData[i].RLWRT;
					}
				}
			}
			poItemDetail.updateBindings();
		...
		handleDeleteAsset: function (oEvent) {
			...
			var poItemData = poItemModel.getProperty("/");
			var poItemDetail = that.getView().getModel("poItemDetail"); //++CH03
			var poItemDetailData = poItemDetail.getData(); //++CH03
			for (var i = 0; i < poItemData.length; i++) {
				if (parseInt(poItemData[i].EBELP, 0) === parseInt(poItem, 0)) { //++CH03
			...
			poItemDetail.updateBindings(); //++CH03
			poItemModel.updateBindings();
		...
		getData: function (po, poItem) {
			...
			aDefer.push(that.getPoItemData(po, poItem));
			$.when.apply($, aDefer).then(function (status) {
				that.updateData(po, poItem);
				that.prepareRefDoc();
				if (that.getView().getBusy() === true) {
					that.getView().setBusy(false);
				}
		...
		updateData: function (po, poItem) {

			var poItemDetail = this.C_MODEL.poItemDetail;

			if (this.getOwnerComponent().getModel("poItem")) {
				var vIndex = this.getDataIndex(po, poItem);
				this.getOwnerComponent().getModel("poItem").setProperty("/" + vIndex, poItemDetail.getData())
			}

		},

		getDataIndex: function (po, poItem) {
			var poItemData = this.getOwnerComponent().getModel("poItem").getProperty("/");
			var vIndex = -1;
			for (var i = 0; i < poItemData.length; i++) {
				if (parseInt(poItemData[i].EBELP, 0) === parseInt(poItem, 0) && parseInt(poItemData[i].EBELN, 0) === parseInt(po, 0)) {
					vIndex = i;
				}
			}
			return vIndex;
		...
		getPoItemData: function (po, poItem) {

			var that = this;
			var srvModel = this.getView().getModel();
			var poItemDetail = this.C_MODEL.poItemDetail;
			var poItemText = this.C_MODEL.poItemText;
			var poAccountAssignment = this.C_MODEL.poAccountAssignment;
			var defer = new $.Deferred();

			var path = "/PoItemSet(EBELN='" + po + "',EBELP='" + poItem + "')";
			...
			srvModel.read(path, {
				...
				success: function (s) {
					...
					if (it.EINDT) it.EINDT.setHours(0);
					...
					poItemDetail.setData({});
					poItemDetail.setData(it);

					for (var i = 0; i < s.PoAccAssignment.results.length; i++) {
						s.PoAccAssignment.results[i].EBELN = parseInt(s.PoAccAssignment.results[i].EBELN, 0);
						s.PoAccAssignment.results[i].EBELP = parseInt(s.PoAccAssignment.results[i].EBELP, 0);
					}

					// PO Item Account Assignment Data
					poAccountAssignment.setData([]);
					poAccountAssignment.setData(s.PoAccAssignment.results);

					// PO Item Text Data
					poItemText.setData([]);
					poItemText.setData(s.PoItemText.results);
		...
		handleItemChange: function (oEvent) {
			...
			return new Promise(function (fnResolve) {
				this.doNavigate("PoItem", "", fnResolve, "", po, poItem);
		...
		handleGoToNextItem: function () {
			...
			return new Promise(function (fnResolve) {
				this.doNavigate("PoItem", "", fnResolve, "", po, nextPoItem);
			}.bind(this)).catch(function (err) {
				if (err !== undefined) {
					MessageBox.error(err.message);
				}
		...
		handleChangeAccountAssignment: function (oEvent) {
			...
			if (selectedKey === "K") {
				...
				temp.push({
					"ZEKKN": "1",
					"KOKRS": "1000",
					"KOKRS_NAME": "เขตการควบคุมจุฬาฯ",
					GSBER: plant,
					GSBER_NAME: plantName
				});
				this.C_MODEL.poAccountAssignment.setProperty("/", temp);
				this.C_MODEL.poItemDetail.setProperty("/KNTTP_NAME", this.getView().getModel("i18n").getProperty(
					"Label.AccountAssignment.CostCenter"));
		...
		syncErrorMessages: function () {
			...
			if (this.getOwnerComponent().getModel("errors")) {
		...
		_getFormFragment: function (sFragmentName) {
			...
			oFormFragment = sap.ui.xmlfragment(this.getView().getId(), "com.cu.s4hana.zlpuba002.view.fragment.PoItem" + sFragmentName, this);
		...
		_showFormFragment: function (sFragmentName) {
			var oPage = this.byId("idPoItemPage");
		...
		doNavBack: function () {
			...
			return new Promise(function (fnResolve) {
				this.doNavigate(page, "", fnResolve, "", po, "");
			}.bind(this)).catch(function (err) {
				if (err !== undefined) {
					MessageBox.error(err.message);
				}
		...
		prepareRefDoc: function () {
			...
			var form = this.getView().byId("idPoItemPage").getContent()[0].getSections()[0].getSubSections()[0].getBlocks()[0].getContent()[0];
			var control = form.getFormContainers()[2].getFormElements()[2].getFields()[0];
			control.removeAllItems();

			if (data.KONNR && data.KONNR !== "") {
				var link = new sap.m.Link({
					text: data.KONNR + " / " + parseInt(data.KTPNR, 0),
					href: that.formatter.parseRefDocLink(data.KONNR, data.KTPNR),
					target: "_blank"
				});
				control.addItem(link);
			} else if (data.BANFN && data.BANFN !== "") {
				var link = new sap.m.Link({
					text: data.BANFN + " / " + parseInt(data.BNFPO, 0),
					href: that.formatter.parseRefDocLink(data.BANFN, data.BNFPO),
					target: "_blank"
				});
				control.addItem(link);
			} else {
		...
		onChangeAsset: function (oEvent) {
			...
			// var path = "/F4AssetSet('" + asset + "')";					// CH06 : DEL
			var path = "/F4AssetSet(ANLN1='" + asset + "',ANLN2='" + subasset + "')";	// CH06 : INS
		...
		selectedSuggestMaterial: function (oEvent) {
			var selectedMatDesc = oEvent.getParameters().selectedRow.getBindingContext("f4Material").getObject().MAKTX;
			this.C_MODEL.poItemDetail.setProperty("/TXZ01", selectedMatDesc);
		},

		selectedSuggestVendor: function (oEvent) {
			var selectedVendorDesc = oEvent.getParameters().selectedRow.getBindingContext("f4Vendor").getObject().NAME1;
			this.C_MODEL.poItemDetail.setProperty("/LIFNR_NAME", selectedVendorDesc);
		},
